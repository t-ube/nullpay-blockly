 ...
 */
export function deserialize(json, interpreter) {
  ...
  objectHunt_(stack);
  const functionMap = Object.create(null);
...
function decodeLoc_(locText) {
  const loc = new NODE_LOC_CONSTRUCTOR();
  const m = locText.match(LOC_REGEX);
  let locStart = null;
  if (m[1] || m[2]) {
...
function populateObject_(jsonObj, obj) {
  ...
  if (props) {
    const nonConfigurable = jsonObj['nonConfigurable'] || [];
    const nonEnumerable = jsonObj['nonEnumerable'] || [];
    const nonWritable = jsonObj['nonWritable'] || [];
    const getter = jsonObj['getter'] || [];
    const setter = jsonObj['setter'] || [];
    ...
    for (let j = 0; j < names.length; j++) {
      ...
      const hasGetter = getter.indexOf(name) !== -1;
      const hasSetter = setter.indexOf(name) !== -1;
 ...
 */
export function serialize(interpreter) {
...
function encodeValue_(value) {
  if (value && (typeof value === 'object' || typeof value === 'function')) {
    const ref = objectMap ? objectMap.get(value) : objectList.indexOf(value);
