...
export const xrpl_payload_nftoken_mint_offer: any = {
  "type": "xrpl_payload_nftoken_mint_offer",
  "message0": "NFToken Mint Offer",
  "message1": "Account %1",
  "args1": [
    {
      "type": "input_value",
...
export const defineXrplNFTokenMintOfferBlock = () => {
  Blockly.defineBlocksWithJsonArray([
    xrpl_payload_nftoken_mint_offer
  ]);

  // JavaScript code generator for the NFTokenMintOffer block
  javascriptGenerator.forBlock['xrpl_payload_nftoken_mint_offer'] = function(block, generator) {
    const account = generator.valueToCode(block, 'ACCOUNT', Order.ATOMIC) || '""';
    const nftokenTaxon = generator.valueToCode(block, 'NFTOKEN_TAXON', Order.ATOMIC) || '0';
    const uri = generator.valueToCode(block, 'URI', Order.ATOMIC) || '""';
    ...
    const code = `xrplNFTokenMintOfferTxn(${account}, ${nftokenTaxon}, ${uri}, ${flags}, ${transferFee}, ${dest},String(${amount}))`;
    return [code, Order.ATOMIC];
  };
};

export function initInterpreterXrplNFTokenMintOfferTxn(interpreter: any, globalObject: any) {
  javascriptGenerator.addReservedWords('xrplNFTokenMintOfferTxn');
  const wrapper = function (account: string, nftokenTaxon: number, uri: string, flags: number, transferFee: number, dest: string, amount: string) {
    const transaction = {
      TransactionType: 'NFTokenMint',
      Account: account,
      NFTokenTaxon: nftokenTaxon,
  ...
  interpreter.setProperty(globalObject, 'xrplNFTokenMintOfferTxn', interpreter.createNativeFunction(wrapper));
}
