...
/** Type of a block that has the DYNAMIC_PARAM_CREATE_MIXIN. */
type DynamicParamCreateBlock = Blockly.Block & DynamicParamCreateMixin;
/** This interface avoids a "circular reference" compile error. */
interface DynamicParamCreateMixin extends DynamicParamCreateMixinType {}
type DynamicParamCreateMixinType = typeof DYNAMIC_PARAM_CREATE_MIXIN;

const DYNAMIC_PARAM_CREATE_MIXIN = {
  ...
  init(this: DynamicParamCreateBlock): void {
    ...
    for (let i = 1; i < this.minInputs; i++) {
      this.appendValueInput(`ADD${i}`).setCheck(blockCheckType.urlParam);
    }
    this.setOutput(true, blockCheckType.string);
    this.setTooltip('Create URL parameters');
  },
   ...
   */
  mutationToDom(this: DynamicParamCreateBlock): Element {
   ...
   */
  domToMutation(this: DynamicParamCreateBlock, xmlElement: Element): void {
   ...
   * @param xmlElement XML storage element.
   */
  deserializeInputs(this: DynamicParamCreateBlock, xmlElement: Element): void {
    ...
    if (items) {
      ...
      this.inputList[0].appendField('URL Parameters');
    }
  },
   ...
   * @param xmlElement XML storage element.
   */
  deserializeCounts(this: DynamicParamCreateBlock, xmlElement: Element): void {
   ...
   */
  saveExtraState: function (this: DynamicParamCreateBlock): { itemCount: number } {
  ...
  onPendingConnection(
    ...
    this.appendValueInput(`ADD${Blockly.utils.idGenerator.genUid()}`).setCheck(blockCheckType.urlParam);
   ...
   */
  finalizeConnections(this: DynamicParamCreateBlock): void {
  ...
  tearDownBlock(this: DynamicParamCreateBlock): void {
  ...
  addItemInputs(
    ...
    for (let i = 1; i < targetConns.length; i++) {
      const input = this.appendValueInput(`ADD${i}`).setCheck(blockCheckType.urlParam);
   ...
   */
  addFirstInput(this: DynamicParamCreateBlock): Blockly.Input {
    return this.appendValueInput('ADD0').appendField('URL Parameters');
  },
   ...
   * False otherwise.
   */
  isCorrectlyFormatted(this: DynamicParamCreateBlock): boolean {
    for (let i = 0; i < this.inputList.length; i++) {
...
Blockly.Blocks['webapi_url_params'] = DYNAMIC_PARAM_CREATE_MIXIN;
